...
namespace p2c::win
{
	...
	{
		std::wstring name{ UiProcessMutexPrefix };
		if (suffix.empty()) {
			suffix = DefaultUiProcessMutexSuffix;
		}
		name.append(suffix.begin(), suffix.end());
		return name;
	}

	inline bool IsUiProcessActive(std::string_view mutexSuffix)
	{
		const auto mutexName = MakeUiProcessMutexName(mutexSuffix);
		::pmon::util::win::Handle hMutex{ OpenMutexW(SYNCHRONIZE, FALSE, mutexName.c_str()) };
		return bool(hMutex);
	}

	inline bool UiWindowMutexSuffixMatches(HWND hWnd, std::string_view mutexSuffix)
	{
		if (mutexSuffix.empty()) {
			mutexSuffix = DefaultUiProcessMutexSuffix;
		}
		const auto suffixAtom = (ATOM)reinterpret_cast<ULONG_PTR>(GetPropW(hWnd, UiWindowMutexSuffixProperty));
		if (suffixAtom == 0) {
			return false;
		}
	...
	inline BOOL CALLBACK FindUiWindowCallback(HWND hWnd, LPARAM lParam)
	{
		...
		if (GetWindowTextW(hWnd, title.data(), (int)title.size()) == 0 ||
			std::wstring_view{ title.data() } != UiWindowTitle) {
			return TRUE;
		}

		if (!UiWindowMutexSuffixMatches(hWnd, params.first)) {
			return TRUE;
		}
	...
	inline HWND FindUiWindow(std::string_view mutexSuffix)
	{
		std::pair<std::string_view, HWND> params{ mutexSuffix, nullptr };
		EnumWindows(FindUiWindowCallback, reinterpret_cast<LPARAM>(&params));
		return params.second;
	}

	inline DWORD FindUiWindowProcessId(std::string_view mutexSuffix)
	{
		const auto hWnd = FindUiWindow(mutexSuffix);
		if (hWnd == nullptr) {
			return 0;
		}
		DWORD processId = 0;
	...
	{
		const auto uiProcessId = FindUiWindowProcessId(mutexSuffix);
		if (uiProcessId == 0) {
			return 0;
		}
	...
	inline bool BringUiWindowToFront(std::string_view mutexSuffix)
	{
		const auto hWnd = FindUiWindow(mutexSuffix);
		if (hWnd == nullptr) {
			return false;
